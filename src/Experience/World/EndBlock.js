...
export default class EndBlock {
  constructor(positionZ, wallMaterial, scoreBoxMaterial, spinnerMaterial) {
    ...
    this.spinner = OBJECTS_PARTS.OBSTACLE.ENDSPINNER;
    this.physicsObjects = [];
    this.bodiesToUpdate = [];
    this.meshesToUpdate = [];
  ...
  contructLastBlock(positionZ, wallMaterial) {
    ...
    const spinners1 = this.createSpinners(positionZ, -4.7);
    const spinners2 = this.createSpinners(positionZ, 4.7);
  ...
  createSpinners(positionZ, positionX) {
    // MESH
    const geometry = new BufferGeometry();
    const material = new MeshBasicMaterial({ color: 0x7708ff });
    const spinnerGeometryArray = new Float32Array([...this.spinner.position]);
    const spinnerMesh = this.constructModel(
      geometry,
      material,
      spinnerGeometryArray
    );
    spinnerMesh.position.set(positionX, 12, positionZ);
    this.meshesToUpdate.push(spinnerMesh);
    // BODY
    const spinnerBody = getPhysicsBody(
      spinnerMesh,
      ShapeType.MESH,
      this.spinnerMaterial,
      0
    );
    this.physicsWorld.addBody(spinnerBody);
    spinnerBody.position.copy(spinnerMesh.position);
    this.bodiesToUpdate.push(spinnerBody);
    return spinnerMesh;
  }
